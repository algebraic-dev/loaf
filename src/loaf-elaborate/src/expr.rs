...
pub fn infer(ctx: &mut Context, ast: &Expr) -> Result<(Rc<Term>, Rc<Value>), ElaborationError> {
    match ast {
        ...
        Expr::Var(vari) => match ctx.types.get(&vari.name.name) {
            ...
            None => {
                match ctx.top_level_ty.get(&vari.name.name) {
                    Some((res_ty, lvl)) => Ok((
                        Rc::new(Term::Top(Top {
                            range: Span::Localized(vari.range.clone()),
                            level: *lvl,
            ...
            for arg in &app.spine {
                match &*force(ctx, res_ty.clone()) {
                    ...
                    _ => {
                        ...
                        return Err(ElaborationError::ExpectedType(ctx.clone(), BinderKind::PiType, arg.locate(), res_ty.quote(ctx.env.depth)))
                    },
                }
            }
            Ok((
        ...
        Expr::Left(left) => {
            ...
            match &*force(ctx, typ_res.clone()) {
                ...
                _ => {
                    Err(ElaborationError::ExpectedType(ctx.clone(), BinderKind::SigmaType, left.expr.locate(), typ_res.quote(ctx.env.depth)))
                },
            }
        }
        Expr::Right(right) => {
            ...
            match &*force(ctx, typ_res.clone()) {
                ...
                _ => Err(ElaborationError::ExpectedType(ctx.clone(), BinderKind::SigmaType, right.expr.locate(), typ_res.quote(ctx.env.depth))),
            }
        }
        Expr::Ann(ann) => {
